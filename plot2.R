...
# Remove NAs, if any
df.plot <- subset( df, !is.na(Global_active_power), select=c(Date, Time, Global_active_power) )
df.plot$DateTime <- as.POSIXct(paste(df.plot$Date,df.plot$Time), format = "%d/%m/%Y %H:%M:%S")

# Plot2: a timeseries based on Date+Time and plotting Global Active Power
